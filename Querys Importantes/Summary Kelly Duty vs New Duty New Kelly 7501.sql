...
FROM(
    ...
    FROM [dbo].[TB_Transfer_TablaKelly_AfterPSC] AS TK WITH(NOLOCK)
    LEFT JOIN
    (
        SELECT DISTINCT  
            SACKellyGlobal			
...
FROM (
    ...
    FROM [dbo].[TB_Transfer_CuadreCI_KellyGlobal_AfterPSC] AS CI WITH(NOLOCK)
    GROUP BY
...
(
    SELECT
        ...
        --select *
    FROM [dbo].[TB_Transfer_Validation_allExport_AfterPSC] AS AE WITH(NOLOCK)
...
FULL JOIN #TB_AllExport_New AS TBA ON TBA.[Entry#] = TBC.[Entry#] AND TBA.[EntryDate] = TBC.[EntryDate] AND TBA.Waybill = TBC.Waybill AND TBA.TariffCategory = TBC.TypeData
ORDER BY 
...
DROP TABLE IF EXISTS AppsLCA.dbo.TB_Transfer_SummaryNewCIKelly_AfterPSC

SELECT * 
INTO AppsLCA.dbo.TB_Transfer_SummaryNewCIKelly_AfterPSC
FROM #TB_Summary
