...
DROP TABLE IF EXISTS #TB_FAMO_SUMMARY
    ...
    INNER JOIN #TB_Group_Waybills_9802 AS B ON AF.ShipDate >= B.ShipDate 
    LEFT JOIN [AppsLCA].[dbo].[TB_Transfer_Export_Duty] AS E WITH(NOLOCK) ON E.ID = AF.ID
...
FROM #TB_Transfer_Trazabilidad AS S
		...
		UPDATE S SET
			[TariffCategory]        = CASE
										WHEN S.[TypeQueryData]      = 1                                     THEN '9802'
										WHEN S.[FAMOTariffCategory] = 'CAFTA'                               THEN 'CAFTA'
										WHEN S.[FAMOTariffCategory] IN( 'NonCAFTA','Non CAFTA','NONCAFTA')  THEN 'NONCAFTA'
										WHEN S.[FAMOTariffCategory] IS NULL AND S.[SeasonName] = 'EMB FG'   THEN 'NONCAFTA'
		...
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'El Salvador'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = '9802' AND [Style] = 'BT300' AND [Manufacturer] = 'L2 BRANDS'
		
		
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'India'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = 'NONCAFTA' AND [Style] in('05PDT','10PDT','15PDT')  AND [Manufacturer] IS NULL
										
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'Pakistan'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = 'NONCAFTA' AND [RO] = '16131-32022-WHT-1'  AND [Manufacturer] IS NULL
		
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'El Salvador'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = 'NONCAFTA' AND [RO] = '16729-20058-MUH-1'  AND [Manufacturer] IS NULL
		
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'Honduras'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = 'NONCAFTA' AND [RO] = '16730-EZ100-LAB'  AND [Manufacturer] IS NULL
		
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'Pakistan'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = 'NONCAFTA' AND [RO] = '19084-144-SYH-1'  AND [Manufacturer] IS NULL
		
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'Pakistan'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = 'NONCAFTA' AND [RO] = '19547-31014-485-1'  AND [Manufacturer] IS NULL
		
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'Guatemala'
			,[TariffCategory] = 'CAFTA'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = 'NONCAFTA' AND [MO] = 'EO3640720-416'  AND [Manufacturer] IS NULL
										
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'Philippines'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = 'NONCAFTA' AND [MO] = 'EO3742877-ROY'  AND [Manufacturer] IS NULL
										
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'China'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = 'NONCAFTA' AND [PoNumber] = 'PO0830O-STOCK-LCA'  AND [Manufacturer] IS NULL
		
		UPDATE S SET
			[FAMOCountryOfOrigin] = 'Guatemala'
		FROM #TB_Transfer_Trazabilidad AS S
		WHERE [TariffCategory] = 'NONCAFTA' AND [ManufactureID] = 488559  AND [Manufacturer] IS NULL
...
UPDATE S SET
	...
	[HTS_%]        = CASE 
						WHEN S.[TariffCategory] = 'CAFTA' THEN 0.0000
...
UPDATE S SET
	 [301China_$]           = IIF(S.[TariffCategory] = '9802', S.[301China_%]     * S.[Export Value Added] , 0.0000)
	,[Fenta_$]              = IIF(S.[TariffCategory] = '9802', S.[Fenta_%]        * S.[Export Value Added] , 0.0000)
	,[Recip_$]              = IIF(S.[TariffCategory] = '9802', S.[Recip_%]        * S.[Export Value Added] , 0.0000)
	,[HTS_$]                = IIF(S.[TariffCategory] = '9802', S.[HTS_%]          * S.[Export Value Added] , 0.0000)
...
UPDATE S SET
	[Drawback]          = IIF(S.[TariffCategory] = '9802', [T_Total_$] - [Total_$],0.0000)
