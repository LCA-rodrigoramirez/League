...
AND CI.Waybill NOT IN (SELECT DISTINCT Waybill FROM [192.168.1.93].AppsLCA.dbo.CI_Import_Export_DeclarationExport_Drawback AS CI WITH(NOLOCK))
...
AND CI.Waybill NOT IN (SELECT DISTINCT Waybill FROM [192.168.1.93].AppsLCA.dbo.CI_Import_Export_CommercialInvoice_Drawback AS CI WITH(NOLOCK))
...
SELECT
	 [Waybill]				= CI.[Waybill]
	,[TypeData]				= CASE
									WHEN CI.[Orden] = 1 THEN 'CAFTA'
									WHEN CI.[Orden] = 2 THEN 'NO CAFTA'
								END
...
(
    ...
    FROM [AppsLCA].[dbo].[TB_Transfer_WaybillEntry_AfterPSC] AS B WITH(NOLOCK)
...
(
    ...
    -- TRUNCATE TABLE AppsLCA.dbo.TB_Transfer_CuadreCI_KellyGlobal_AfterPSC
    -- INSERT INTO AppsLCA.dbo.TB_Transfer_CuadreCI_KellyGlobal_AfterPSC

    DROP TABLE IF EXISTS AppsLCA.dbo.TB_Transfer_CuadreCI_KellyGlobal_AfterPSC
	SELECT * 
    INTO AppsLCA.dbo.TB_Transfer_CuadreCI_KellyGlobal_AfterPSC
    FROM #TB_DATA_CI
