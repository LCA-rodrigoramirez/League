...
UPDATE S SET
	...
	,[Year_ExportDate]   = YEAR(COALESCE(B.EntryDate ,S.OriginalExportDate))
FROM #TB_Transfer_Trazabilidad AS S
LEFT JOIN [AppsLCA].[dbo].[TB_Transfer_WaybillEntry_AfterPSC] AS B WITH(NOLOCK) ON B.Waybill = S.Waybill
...
DROP TABLE IF EXISTS [AppsLCA].[dbo].[TB_Transfer_Validation_allExport_AfterPSC]
...
,CASE 
 ...
 END AS CountInvoice
INTO [AppsLCA].[dbo].[TB_Transfer_Validation_allExport_AfterPSC]
FROM #TB_Transfer_Trazabilidad 
...
SELECT 
	...
	,KellyReport
	,Original_IDExport
FROM [AppsLCA].[dbo].[TB_Transfer_Validation_allExport_AfterPSC]
WHERE Waybill = 'AIR-HW-20260109' AND TariffCategory = 'NO CAFTA' AND CountryOfOrigin <> FAMOCountryOfOrigin

SELECT SUM(QtyExport), US_HTSCode
FROM [AppsLCA].[dbo].[TB_Transfer_Validation_allExport_AfterPSC]
WHERE Waybill = 'AIR-APP-20251212' AND TariffCategory = 'NO CAFTA RULE 9802'
GROUP BY US_HTSCode
