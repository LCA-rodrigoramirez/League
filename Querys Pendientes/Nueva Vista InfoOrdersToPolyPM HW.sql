...
--ALTER VIEW [dbo].[VW_InfoOrdersToPolyPM]
--AS
--SELECT * FROM [dbo].[VW_InfoOrdersToPolyPM]
...
(
	SELECT
		...
		,od.OrderID
		,od.PONumber
	FROM [192.168.1.53].[AppsLCA].[legacycaps].[VW_view_qryLCA_Order_Export] AS OE WITH(NOLOCK)
	LEFT JOIN 
...
INNER JOIN CTE_ORDERS AS CO WITH(NOLOCK) ON VLD.ItemDetailID = CO.ItemDetailID
			...
			FROM (
				SELECT DISTINCT 
					...
					CASE 
						...
						THEN CONCAT(VVLA.LogoStyle, IIF(CO.Brand LIKE '%Locale%'  AND VVLA.LogoStyle IN ('MIF','MIS','MISL','PX','PXCP','PXFB'),'LOC','') ,'2')
						WHEN CAST (StitchCount as INT) >= 2001 and CAST (StitchCount as INT) <= 4000 
						THEN CONCAT(VVLA.LogoStyle, IIF(CO.Brand LIKE '%Locale%'  AND VVLA.LogoStyle IN ('MIF','MIS','MISL','PX','PXCP','PXFB'),'LOC','') ,'4')
						WHEN CAST (StitchCount as INT) >= 4001 and CAST (StitchCount as INT) <= 6000 
						THEN CONCAT(VVLA.LogoStyle, IIF(CO.Brand LIKE '%Locale%'  AND VVLA.LogoStyle IN ('MIF','MIS','MISL','PX','PXCP','PXFB'),'LOC','') ,'6')
						WHEN CAST (StitchCount as INT) >= 6001 and CAST (StitchCount as INT) <= 8000 
						THEN CONCAT(VVLA.LogoStyle, IIF(CO.Brand LIKE '%Locale%'  AND VVLA.LogoStyle IN ('MIF','MIS','MISL','PX','PXCP','PXFB'),'LOC','') ,'8')
						WHEN CAST (StitchCount as INT) >= 8001 and CAST (StitchCount as INT) <= 10000 
						THEN CONCAT(VVLA.LogoStyle, IIF(CO.Brand LIKE '%Locale%'  AND VVLA.LogoStyle IN ('MIF','MIS','MISL','PX','PXCP','PXFB'),'LOC','') ,'10')
						WHEN CAST (StitchCount as INT) >= 10001 and CAST (StitchCount as INT) <= 12000 
						THEN CONCAT(VVLA.LogoStyle, IIF(CO.Brand LIKE '%Locale%'  AND VVLA.LogoStyle IN ('MIF','MIS','MISL','PX','PXCP','PXFB'),'LOC','') ,'12')
						WHEN CAST (StitchCount as INT) >= 12001 and CAST (StitchCount as INT) <= 14000 
						THEN CONCAT(VVLA.LogoStyle, IIF(CO.Brand LIKE '%Locale%'  AND VVLA.LogoStyle IN ('MIF','MIS','MISL','PX','PXCP','PXFB'),'LOC','') ,'14')
						WHEN CAST (StitchCount as INT) >= 14001 and CAST (StitchCount as INT) <= 16000 
						THEN CONCAT(VVLA.LogoStyle, IIF(CO.Brand LIKE '%Locale%'  AND VVLA.LogoStyle IN ('MIF','MIS','MISL','PX','PXCP','PXFB'),'LOC','') ,'16')
						WHEN StitchCount IS NULL THEN VVLA.LogoStyle
						ELSE CONCAT(VVLA.LogoStyle, IIF(CO.Brand LIKE '%Locale%'  AND VVLA.LogoStyle IN ('MIF','MIS','MISL','PX','PXCP','PXFB'),'LOC','') ,'16')
					END AS Code
				FROM CTE_ORDERS AS CO WITH(NOLOCK) 
