USE [LCA]
GO
/****** Object:  StoredProcedure [dbo].[InsertBillingDetails_Each]    Script Date: 27/05/2025 05:25:21 p. m. ******/
SET ANSI_NULLS ON
GO
...
-- Declare @VWaybill varchar(200) = 'AIR-BUND-20250530'-- 'AIR-BUND-20240820' --'AIR-BUND-20240621-NO CAFTA'  
...
Select distinct 
	...
	from #TBoxes3 ABC10
		...
		left join (SELECT Style_Blank, Style_EMB, StyleID_Emb, StyleID_Blank
									FROM [LCA].[dboReaders].[VW_ESC_StyleStructure] with (nolock) WHERE Style_EMB <> Style_BLANK
					) BlankStyle
			on ABC10.StyleNumber = BlankStyle.Style_EMB
...
--select * from #ABC11_Pre order by boxnumber, stylenumber, Stylecolor, GarmentSize
...
-- Select * from #TBoxes2 --where boxnumber ='00786738'
...
Select distinct	 ABC2.ShipDate, 
	...
	case when substring(ABC2.Waybill,1,2)='SM' then isnull(ABC3.BasePrice,ABC4.UnitPrice)
		 when CHARINDEX('FG',SeasonName)>0  
			then ABC4.UnitPrice
				---ScreenPrint
					- case when ABC2.PrintCount is null or ABC2.PrintCount='' then 0 	else  ABC3.TotalPrintValue end
				---Sublimation
	...
	case when ABC2.PrintCount is null or ABC2.PrintCount='' then 0 
		else  ABC3.TotalPrintValue end as Screen_Print, 

	ABC2.QTY *  case when ABC2.PrintCount is null or ABC2.PrintCount='' then 0 
		else  ABC3.TotalPrintValue end as Total_Screen_Print,
	
	case when charindex('To Embro',ABC2.TypeOrd)>0 then ABC2.Embr_Total else 0  end as Embroidery,
	ABC2.QTY * case when charindex('To Embro',ABC2.TypeOrd)>0 then ABC2.Embr_Total else 0  end as Total_Embroidery,
	
	case when charindex('To Subli',ABC2.TypeOrd)>0 then ABC2.Total_Sp_Subli else 0  end as Sublimation,
	ABC2.QTY * case when charindex('To Subli',ABC2.TypeOrd)>0 then ABC2.Total_Sp_Subli else 0  end as Total_Sublimation,

	case when substring(ABC2.Waybill,1,2)='SM' then isnull(ABC3.BasePrice,ABC4.UnitPrice)
		 when charindex('bund',ABC2.Waybill)=0 and ABC4.UnitPrice <> ABC3.Price then ABC4.UnitPrice
	...
	case when substring(ABC2.Waybill,1,2)='SM' then isnull(ABC3.BasePrice,ABC4.UnitPrice) * ABC2.Qty
		 when charindex('bund',ABC2.Waybill)=0 and  ABC4.UnitPrice <> ABC3.Price	then ABC4.UnitPrice * ABC2.Qty
...
---Apparel
	...
	, case when  abc4_3.metodo=1
			then (case when substring(ABC2.Waybill,1,2)='SM' then isnull(ABC3.BasePrice,ABC4.UnitPrice) * ABC2.Qty
						when ABC4.UnitPrice <> ABC3.Price then ABC4.UnitPrice * ABC2.Qty
	...
	,(select distinct shp.ContainerNumber  
		from lca.dbo.ShippingContainers  Cont with (nolock)
			left outer join  lca.dbo.Shipments shp with (nolock)
				...
				where shp.WayBill = REPLACE(REPLACE(REPLACE(RTRIM(isnull(ABC4.Waybill , ABC2.Waybill)),CHAR(9), ''),CHAR(10),''),CHAR(13),'')) as Container
	,0 as TempGrossWG
	,0 as TempoNetWeight
	,0 as TempQty
	,Cuenta
...
update BDS set BDS.Supplier = TM.Vendor
	FROM #Report  BDs 
	inner join AppsLCA.[dbo].[TB_MO_PartNumber_IM_summary]	 TM with (nolock)
		on BDS.ManufactureID = TM.ManufactureId
	where TM.Category='Contracts'

--select * from #Report -- WHERE HTSDescription is null or HTSDescription =''
--select * from #TBoxes2
...
if @Find_WayBill >0
	...
	--select * from #Report where BoxNumber = '00879373'
