USE [Financial]
GO
/****** Object:  StoredProcedure [dbo].[InsertBillingDetails_Each]    Script Date: 21/09/2026 03:46:42 p. m. ******/
SET ANSI_NULLS ON
GO
...
DROP TABLE IF EXISTS #TBoxes1

DROP TABLE IF EXISTS #TBoxes1_BAK
DROP TABLE IF EXISTS #UnitsOriginal_1
DROP TABLE IF EXISTS #UnitsOriginal_2
DROP TABLE IF EXISTS #CajasOriginal

DROP TABLE IF EXISTS #HWEmbroidery
DROP TABLE IF EXISTS #TBoxes2
DROP TABLE IF EXISTS #TBaseCost
DROP TABLE IF EXISTS #TBaseCost_Fabric
DROP TABLE IF EXISTS #TPartNumber
DROP TABLE IF EXISTS #TBoxes3
DROP TABLE IF EXISTS #TInvoiceBatch
DROP TABLE IF EXISTS #TStdCost_Full
DROP TABLE IF EXISTS #TStdCost_FG
DROP TABLE IF EXISTS #Report	
DROP TABLE IF EXISTS #TCostP1	
DROP TABLE IF EXISTS #TPurch	
DROP TABLE IF EXISTS #TSales_PNew	
DROP TABLE IF EXISTS #abc4_1
DROP TABLE IF EXISTS #abc4_2

DROP TABLE IF EXISTS #Tboxes4
DROP TABLE IF EXISTS #Tboxes5
DROP TABLE IF EXISTS #TEmbrCodes

DROP TABLE IF EXISTS #abc4_3
DROP TABLE IF EXISTS #BoxList
DROP TABLE IF EXISTS #T_VW_BoxTransactions
DROP TABLE IF EXISTS #OrigFab
DROP TABLE IF EXISTS #ABC11_Pre
DROP TABLE IF EXISTS #SalesEmbroidery

----------- NUEVAS TABLAS PARA GUARDAR EL SAM DE COSTURA ------------------------
DROP TABLE IF EXISTS #TB_LABOR
DROP TABLE IF EXISTS #CTE_LABOR


DECLARE @DateNewFreight AS DATE = '2027-01-01'
--Declare @VFecha date =    CONVERT(DATE, GETDATE() -4 ) 
Declare @VFecha  date = '2026-08-01'
Declare @VFecha2 date = '2026-08-31'

--  Declare @VWaybill varchar(200) = 'AIR-HW-20260629'-- 'AIR-BUND-20240820' --'AIR-BUND-20240621-NO CAFTA'  
...
Select 
	...
	SMO.MOID as ManufactureID, isnull(SMO.ProductDivision,'Apparel') as ProductDivision, null as RO, null as RO_ID, IE_Ship.FormattedBoxNumber
	from [Financial].[dbo].[VW_Check_Sales_Prices_in_Invoices_SeekMO_3] SMO with (nolock)
	    left outer join (
						select distinct ShipDate, WayBill, InvoiceBatch, Batch, BoxNUmber, FormattedBoxNumber
		...
		left outer join (
							select distinct ShipDate, WayBill, InvoiceBatch, Batch, BoxNUmber, StyleNumber, StyleColor, GarmentSize, CA_HTSDescription
								from 
									[Financial].[dboReaders].[VW_ImpExp_ShipmentBoxItems_CheckPrice] CHP2  with (nolock)
									where BoxNumber in  (select BoxNumber from #BoxList) --in ('00673224')
						) CHP  
...
Select 
	...
	SMO.Quantity as Qty, SMO.OrderID, SMO.EmbroideryCode, SMO.Size as GarmentSize,
	SMO.MOID as ManufactureID, isnull(SMO.ProductDivision,'Apparel') as ProductDivision, null as RO, null as RO_ID, IE_Ship.FormattedBoxNumber
	from [Financial].[dbo].[VW_Check_Sales_Prices_in_Invoices_SeekMO_Bundle] SMO with (nolock)
	    left outer join (
						select distinct ShipDate, WayBill, InvoiceBatch, Batch, BoxNUmber, FormattedBoxNumber
							from appslca.dbo.[ImportExport_ShipmentBoxAll]  with (nolock)
							where BoxNumber in  (select BoxNumber from #BoxList) --in ('00673224')
...
select * from [Financial].[dboReaders].[VW_BoxTransactions] with (nolock) where Attached_ID in (select distinct ManufactureID from #TBoxes1)
...
--Select  Quantity as Qty, ManufactureNumber as RO, ManufactureID as RO_ID, AttachedMO as EO, Attached_ID as EO_ID, 
	...
	from [Financial].[dboReaders].[VW_BoxTransactions] with (nolock)
	where concat(AttachedMO, Attached_ID, Size) in 
				...
				(select row_number() over(Partition by AttachedMO, Size order by AttachedMO, Size ) as Ncuenta,
					Qty, ManufactureNumber as RO, ManufactureID  as RO_ID, AttachedMO as EO, Attached_ID as EO_ID,  Size
						...
						(select ManufactureNumber, ManufactureID,Size, sum(Quantity) as Qty, AttachedMO, Attached_ID
								...
								and VW_BT.id not in (11049880,11049898,11049896,11053748,11053751,13723193,13723445) 
		...
		and id not in (11305418,11305427,11305421,11305424,11305413,11305410,11305416,11349189,11349190,11349188,11349187,11349185,11349186,11349191,13723193,13723445)
...
select distinct Boxnumber, StyleNumber, GarmentSize, sum(qty) as qty,  ManufactureID as EO_ID, StyleNumber as EO_Style, FormattedBoxNumber
	from #TBoxes1
	group by Boxnumber, StyleNumber, GarmentSize, ManufactureID, STyleNumber, FormattedBoxNumber
...
declare @FormattedBoxNumber varchar(200)
...
declare @CO_FormattedBoxNumber varchar(200)
	...
	begin
		...
		open C_AsignUnid
				Fetch next from C_AsignUnid into @Waybill, @HTSDescription, @ShipDate, @InvoiceBatch, @Batch,  @PONumber, @BoxNumber, @StyleNumber, @SeasonName, @StyleColor,
													@Qty, @OrderID, @EmbroiderCode, @GarmentSize, @ManufactureID, @ProductDivision, @RO, @RO_ID, @FormattedBoxNumber
					while @@FETCH_STATUS=0
					begin
						...
						open C_CajaOriginal
						Fetch next from C_CajaOriginal into @CO_Boxnumber, @CO_StyleNumber, @CO_GarmentSize, @CO_Qty, @CO_EO_ID, @CO_Style, @CO_FormattedBoxNumber
									...
									if @CU_1_Qty < = @CO_Qty 
										begin
											insert into #TBoxes1 values (@Waybill, @HTSDescription, @ShipDate, @InvoiceBatch, @Batch,  @PONumber, @CO_Boxnumber, @StyleNumber,
																		...
																		@ProductDivision, @CU_1_RO, @CU_1_RO_ID, @CO_FormattedBoxNumber)
...
--Proceso para cargar codigo de embroidery para Bordado de HW			   
	...
	select distinct ABC10.BoxNumber, EMBR1.OrderID, EMBR1.PONumber, null as HWEMB_Code, WF.WorkFlowID, ORD_Det.StyleID, STY.Comments9, TYU.MOID
		from #TBoxes1 ABC10
			...
			left join Financial.dbo.ManufactureOrders MO1 with (nolock)
				on TYU.MOID=MO1.manufactureID and MO1.StatusID <=90
			left join Financial.dbo.Orders EMBR1 with (nolock)
				on MO1.OrderID = EMBR1.OrderID and EMBR1.StatusID <=90 
			left join Financial.dbo.[WorkFlows] WF with (nolock)
				on TYU.MOID = WF.ManufactureID
			left join Financial.dbo.OrderItems ORD_Det with (nolock)
				on MO1.FirstOrderItemID = ORD_Det.OrderItemID
			left join Financial.dbo.Styles STY with (nolock)
	...
	while @@Fetch_status=0
			...
			if @ProductDivision<>'Apparel'
				begin
					Set @Es_HWEmbr1 =(Select count(*) from [Financial].[dbo].[WorkTasks] with (nolock) 
										where TaskName like 'Finish Embroidery  HW 1' and WorkFlowId=@WorkFlowID )
					Set @Es_HWEmbr2 =(Select count(*) from [Financial].[dbo].[WorkTasks] with (nolock) 
										where TaskName like 'Finish Embroidery  HW 2' and WorkFlowId=@WorkFlowID )
					Set @Es_HWEmbr3 =(Select count(*) from [Financial].[dbo].[WorkTasks] with (nolock) 
										where TaskName like 'Finish Embroidery  HW 3' and WorkFlowId=@WorkFlowID )
					Set @VerificaCampo = (select top 1 comments26 from Financial.dbo.Orders  where OrderID = @OrderID)
					...
					if @Es_HWEmbr3>0 and (@VerificaCampo is null or  ltrim(REPLACE(REPLACE(REPLACE(RTRIM(@VerificaCampo),CHAR(9), ''),CHAR(10),''),CHAR(13),'') )='')
						begin
							...
							update Financial.dbo.Orders set Comments26='TBD,TBD,TBD' where OrderID = @OrderID
						end
					else
						begin
							if @Es_HWEmbr2>0 and (@VerificaCampo is null or  ltrim(REPLACE(REPLACE(REPLACE(RTRIM(@VerificaCampo),CHAR(9), ''),CHAR(10),''),CHAR(13),'') )='')
								begin
									...
									update Financial.dbo.Orders set Comments26='TBD,TBD' where OrderID = @OrderID
								end
							else
								begin
									if @Es_HWEmbr1>0 and (@VerificaCampo is null or ltrim(REPLACE(REPLACE(REPLACE(RTRIM(@VerificaCampo),CHAR(9), ''),CHAR(10),''),CHAR(13),'') )='')
										begin
											...
											update Financial.dbo.Orders set Comments26='TBD' where OrderID = @OrderID
										end
								end
						end
...
Create Table  #TBoxes2 (
	...
	--Se agregar cuatro campos mas para las ordenes de HW que pueden llebar hasta 8 codigos de bordado
	--Realizado por Boris Hernan, Rodrigo Ramirez, Juan Jimenez 2026 07 29

	Embr_Code5 varchar(250) null,
	Embr_Code6 varchar(250) null,
	Embr_Code7 varchar(250) null,
	Embr_Code8 varchar(250) null,
...
select 	Waybill, HTSDescription,ShipDate, InvoiceBatch, Batch, PONumber, BoxNumber, StyleNumber, SeasonName, 
		...
		EmbroiderCode, GarmentSize,	ManufactureID, ProductDivision,	RO,	RO_ID, FormattedBoxNumber
	from #TBoxes1
...
create table #TEmbrCodes (
	...
	Embr_Code5 varchar(200) null,
	Embr_Code6 varchar(200) null,
	Embr_Code7 varchar(200) null,
	Embr_Code8 varchar(200) null
...
select orderid, max(case when ColN=1 then [Value] end) as EMBR_Code1,
				...
				max(case when ColN=5 then [Value] end) as EMBR_Code5,
				max(case when ColN=6 then [Value] end) as EMBR_Code6,
				max(case when ColN=7 then [Value] end) as EMBR_Code7,
				max(case when ColN=8 then [Value] end) as EMBR_Code8
				
	from 
		(select OrderId, [value], row_number() over (partition by Orderid order by Orderid)  as ColN
			from 
				(select Orderid, ponumber, value from 
					(select orderid, ponumber , comments26 from Financial.dbo.orders with (nolock)
		...
		(select OrderId, [value], row_number() over (partition by Orderid order by Orderid)  as ColN
			from 
				(select Orderid, ponumber, value from 
					(select orderid, ponumber , comments26 from Financial.dbo.orders with (nolock)
						where orderid in (
										 select distinct Orderid from Financial.dbo.OrderItems 
...
Select
			...
			isnull(NewEmbr.Embr_Code5,isnull(NewEmbr2.Embr_Code5,'')) as Embr_Code5,
			isnull(NewEmbr.Embr_Code6,isnull(NewEmbr2.Embr_Code6,'')) as Embr_Code6,
			isnull(NewEmbr.Embr_Code7,isnull(NewEmbr2.Embr_Code7,'')) as Embr_Code7,
			isnull(NewEmbr.Embr_Code8,isnull(NewEmbr2.Embr_Code8,'')) as Embr_Code8,
	...
	from #TBoxes3 ABC10
		...
		left join (SELECT Style_Blank, Style_EMB, StyleID_Emb, StyleID_Blank, SeasonName_BLANK, SeasonName_EMB
									FROM [Financial].[dboReaders].[VW_ESC_StyleStructure] with (nolock) WHERE Style_EMB <> Style_BLANK
					) BlankStyle
		...
		left join 	[AppsLCA].[dbo].[Financial_TB_MO_PartNumber_IM_Summary] TB_MO89 with (nolock) 
			--Cambio realizado 2026 07 29 por Boris Her, Rodrigo Ram, Juan Jimen
			--on MO1.ManufactureID = TB_MO89.ManufactureID
			-- ISNULL(MO2.ManufactureID,MO1.ManufactureID) = TB_MO89.ManufactureID
			on  iif(MO2.ManufactureID is null or len(MO2.ManufactureID) ='' or MO2.ManufactureID=0, MO1.ManufactureID, MO2.ManufactureID) = TB_MO89.ManufactureID
		...
		left join [AppsLCA].[dbo].[Financial_TB_MO_PartNumber_IM_COO_Per_MO] TB_MO89_2 with (nolock) 
			--Cambio realizado 2026 07 29 por Boris Her, Rodrigo Ram, Juan Jimen
			--on MO1.ManufactureID = TB_MO89_2.ManufactureID
			on  iif(MO2.ManufactureID is null or len(MO2.ManufactureID) ='' or MO2.ManufactureID=0, MO1.ManufactureID, MO2.ManufactureID) = TB_MO89_2.ManufactureID

		left join #TEmbrCodes NewEmbr 
			on MO1.OrderID = NewEmbr.OrderID 
		---Agregando por Waybill Bundle
		left join  [Financial].[dbo].[VW_Check_Sales_Prices_in_Invoices_SeekMO_Bundle]  TYU2 with (nolock)
			on ABC10.BoxNumber = TYU2.BoxNumber
			and ABC10.ManufactureID = TYU2.MOID
		left join Financial.dbo.ManufactureOrders MO1_2 with (nolock)
			on TYU2.MOID=MO1_2.manufactureID 
		left join #TEmbrCodes NewEmbr2
			on MO1_2.OrderID = NewEmbr2.OrderID 
		left join Financial.dbo.Orders EMBR1_2 with (nolock)
			on MO1_2.OrderID = EMBR1_2.OrderID 
		left join [Financial].[dboReaders].[VW_EORO] VWEORO_2 with (nolock)
			on TYU2.MOID = VWEORO_2.EO_ID
		left join Financial.dbo.ManufactureOrders MO2_2 with (nolock)
			on VWEORO_2.RO_ID = MO2_2.ManufactureID 
		left join Financial.dbo.Orders EMBR2_2 with (nolock)
			on MO2_2.OrderID=EMBR2.OrderID 
		left join 	[AppsLCA].[dbo].[Financial_TB_MO_PartNumber_IM_Summary] TB_MO892 with (nolock) 
		...
		left join [AppsLCA].[dbo].[Financial_TB_MO_PartNumber_IM_COO_Per_MO] TB_MO89_2_2 with (nolock) 
...
select distinct ABC11.*,
	...
	---Boris Hernandez y Rodrigo Ramirez 2024 09 11
		(select waybill, HTSDescription, ShipDate, InvoiceBatch, Batch, PONumber, BoxNumber, FormattedBoxNumber, StyleNumber,SeasonName, Stylecolor, sum(qty) as qty,
			OrderID, EmbroideryCode, GarmentSize, ManufactureID,ProductDivision,RO,RO_ID,MO,Embr_Code1,Embr_Code2,Embr_Code3,Embr_Code4,
			Embr_Code5,Embr_Code6,Embr_Code7,Embr_Code8,PrintLocations,
			PrintCount,OrderTypeID2,StyleOptionID,CountryOfOrigin,Manufacturer,Vendor_2
			from (
					select	Waybill , HTSDescription, ShipDate , InvoiceBatch, Batch , PONumber, BoxNumber, FormattedBoxNumber, StyleNumber ,
							SeasonName,Stylecolor,	case	when Original_qty = qty_2 and Original_qty=Qty_pre then qty_2
								...
								RO ,RO_ID ,	MO , Embr_Code1 ,	Embr_Code2 ,	Embr_Code3 ,	Embr_Code4 ,
								Embr_Code5 ,	Embr_Code6 ,	Embr_Code7 ,	Embr_Code8 ,
								 PrintLocations ,PrintCount ,
				...
				) abc79823 group by waybill, HTSDescription, ShipDate, InvoiceBatch, Batch, PONumber, BoxNumber, FormattedBoxNumber, StyleNumber,SeasonName, Stylecolor,
									OrderID, EmbroideryCode, GarmentSize, ManufactureID,ProductDivision,RO,RO_ID,MO,Embr_Code1,Embr_Code2,Embr_Code3,
									Embr_Code4,Embr_Code5,Embr_Code6,Embr_Code7,Embr_Code8,PrintLocations,PrintCount,OrderTypeID2,StyleOptionID,CountryOfOrigin,Manufacturer,Vendor_2
		) ABC11
			...
			left join #SalesEmbroidery SaleEMB5
				on ABC11.Embr_Code5 = SaleEmb5.Code
			left join #SalesEmbroidery SaleEMB6
				on ABC11.Embr_Code6 = SaleEmb6.Code
			left join #SalesEmbroidery SaleEMB7
				on ABC11.Embr_Code7 = SaleEmb7.Code
			left join #SalesEmbroidery SaleEMB8
				on ABC11.Embr_Code8 = SaleEmb8.Code
			...
			left join [Financial].[dbo].[DropDownValues2] DDV2 with (nolock) 
					on ABC11.OrderTypeID2=DDV2.DropDownValueID
			left join [Financial].[dboReaders].[VW_Planning_OrderItemsPriceRev_2] OrdPR with (nolock) 
				on ABC11.ManufactureID = OrdPr.ManufactureID
				and ABC11.PoNumber = OrdPR.Ponumber
				and ABC11.StyleColor = OrdPr.Color
			left join [Financial].[dboReaders].[VW_Planning_OrderItemsPriceRev_3] ORDPR_2 with (nolock)  					--- Vista que trae el precio de Embroidery para los Bundle --- Agregado por Rodrigo Ramirez 20250530
		...
		(	Select EO, row_number() over(Partition by EO order by EO, Units desc) as Cuenta,
				...
				from 
					(Select  VWEORO.EO, VWEORO.RO, sum(VWEORO.UnitsEORO) as Units, VWVendor.OrigFabricVendorName
							from  [Financial].[dboReaders].[VW_EORO] VWEORO with (nolock)
	...
	from 
		...
		Select distinct PoNumber, BoxNumber, Style, EO,  Color,  Size
			...
			(
					...
					from #TBoxes2 TBOX2
					left outer join Appslca.dbo.Financial_TB_MO_PartNumber_IM_EORO VWEORO with (nolock)
...
(select string_agg([value],', ') from (select distinct [value] from string_split(string_agg(PoNumber, ','),',')) t) as Ponumber
  FROM [Financial].[dbo].[VW_Check_Sales_Prices_in_ReceiveSlips_Fabric] with (nolock)
  --'3542086-CPTN'
	...
	from (
			Select ManufactureID,
			 replace(REPLACE(REPLACE(REPLACE(RTRIM(PartNumber),CHAR(9), ''),CHAR(10),''),CHAR(13),''),' ','')
			  as PartNumber, Category, RO_ID from 
			  -- SE AGREGA RO ID POR PARTNUMBER DISTINTOS DESPACHADOS HACIA UNA EO LO CUAL CAUSA DUPLICADOS AL FINAL SI NO SE RELACIONA POR RO -- AGREGADO POR RR Y DP 20260601
				  appslca.dbo.Financial_TB_MO_PartNumber_IM with (nolock)
				where manufactureID  in (select distinct ManufactureID from #TBoxes2)   
			          and category='Contracts' --and Partnumber in ('CFA-WHT-F-ADJ','ML510-809-S','CFV-NAV-F-ADJ  ')
			) abcd
	group by ManufactureID, Category, PartNumber, RO_ID
...
select 
	...
	from appslca.dbo.Financial_TB_MO_PartNumber_IM with (nolock)
...
create table  #Report (
    ...
    ,[InlandFreight] bit null
    ,[AirFreight] bit null
    ,[OceanFreight] bit null
    ,[Price_InlandFreight] decimal(18,4) null
    ,[Price_AirFreight] decimal(18,4) null
    ,[Price_OceanFreight] decimal(18,4) null
    ,[OutboundFreight] decimal(18,4) null
    ,PuertoDestino Varchar(100) null
    ,ShipTo Varchar(100) null
	,[SewingLabor_SAM] decimal(18,4) null
    ,[SewingLaborExpected_SAM] decimal(18,4) null
)
insert into #Tboxes2
select * from #TBoxes4
...
(
	...
	FROM Financial.dbo.Styles			AS ST1 WITH(NOLOCK)
...
Select distinct	 ABC2.ShipDate, 
	 --isnull(ABC4.Waybill , ABC2.Waybill) as Waybill  ,
	  REPLACE(REPLACE(REPLACE(RTRIM(isnull(ABC4.Waybill , ABC2.Waybill)),CHAR(9), ''),CHAR(10),''),CHAR(13),'') 
	   as waybill,
	 ABC2.InvoiceBatch, ABC2.Batch, ABC2.PONumber, ABC2.BoxNumber, ABC2.FormattedBoxNumber ,ABC2.StyleNumber,
	 ABC2.SeasonName,
...
---Apparel
	...
	,(
		...
		from Financial.dbo.ShippingContainers  Cont with (nolock)
			left outer join  Financial.dbo.Shipments shp with (nolock)
    ...
    ,SCPD.AirFreight
    ,SCPD.OceanFreight
    ,SCPD.Price_InlandFreight
    ,SCPD.Price_AirFreight
    ,SCPD.Price_OceanFreight
    ,SCPD.OutboundFreight
    ,SCPD.PuertoDestino
    ,SCPD.ShipTo
	,[SewingLabor_SAM] = CAST(0.00 AS DECIMAL(18,4))
    ,[SewingLaborExpected_SAM] = CAST(0.00 AS DECIMAL(18,4))
	--select *
from  
	(
	...
	left join [AppsLCA].[dbo].[Financial_TB_MO_PartNumber_IM_Materials] TBMAT with (nolock)
		on ABC2.Manufactureid = TBMAT.ManufactureID
	left join appslca.dbo.Financial_TB_MO_PartNumber_IM_Materials TB_sum with (nolock)
	...
	left outer join Financial.dbo.StyleOptions StyOpt with (nolock)
	...
	left join [Financial].[dboReaders].[VW_Planning_OrderItemsPriceRev_3] ORDPR_2 with (nolock)
...
update BDS set BDS.Supplier = ADR.CompanyName
	...
	inner join (select distinct ManufactureID, PONumber, PurchaseID 
						from AppsLCA.dbo.Financial_TB_MO_PartNumber_IM_Freight  with (nolock)
	...
	inner join (select isnull(BilltoID, VendorID) as VendorID, PurchaseID from Financial.dbo.purchaseOrders with (nolock)) POs
		on abc678.PurchaseID = POs.PurchaseID
	inner join Financial.dbo.Addresses ADR with (nolock)
...
from #Report BD
	inner join AppsLCA.dbo.Financial_TB_MO_PartNumber_IM_Materials TB1
		on BD.RO_ID= TB1.ManufactureID
	where TB1.Contracts_PurchasePrice>0


------------ 2026 09 21 Actualizacion de SewingLabor_SAM y SewingLaborExpected_SAM en Anexo Facturacion --------------- AGREGADO POR RR
			SELECT
				 [StyleID]		= STT.StyleID
			...
			INTO #CTE_LABOR
			--SELECT *
			FROM			Financial.dbo.ComponentCategories		AS CC WITH(NOLOCK)
			INNER JOIN		Financial.dbo.ComponentLibrary		AS CL WITH(NOLOCK) ON CL.ComponentCategoryID	= CC.ComponentCategoryID AND CC.ComponentCategoryID IN( 4,6,9,12) --Trim,CutLabor, SewLabor, Handling
			INNER JOIN		Financial.dbo.ComponentSubcategories	AS CB WITH(NOLOCK) ON CB.SubCategoryID			= CL.SubCategoryID
																				AND (
			...
			INNER JOIN		Financial.dbo.StyleDetails			AS SD WITH(NOLOCK) ON SD.ComponentID			= CL.ComponentID
			...
			INNER JOIN	(
							...
							FROM		Financial.dbo.Styles				AS ST1 WITH(NOLOCK)
							LEFT JOIN	Financial.dbo.Styles				AS ST2 WITH(NOLOCK) ON ST1.BlankStyleID		= ST2.StyleID
							LEFT JOIN	Financial.dbo.Seasons				AS SN1 WITH(NOLOCK) ON SN1.SeasonID			= ST1.SeasonID
							LEFT JOIN	Financial.dbo.Seasons				AS SN2 WITH(NOLOCK) ON SN2.SeasonID			= ST2.SeasonID
							LEFT JOIN   Financial.dbo.StatusNames         AS S1  WITH(NOLOCK) ON S1.StatusID          = ST1.StatusID
							LEFT JOIN (
										SELECT
												[StyleID]		= ST.StyleID
										...
										FROM		Financial.dbo.Styles	AS ST	WITH(NOLOCK)
										INNER JOIN	Financial.dbo.Seasons	AS SN	WITH(NOLOCK) ON ST.SeasonID = SN.SeasonID AND SN.SeasonID = 2231	--Season EMB Cost
										LEFT JOIN	Financial.dbo.Styles	AS STB	WITH(NOLOCK) ON ST.StyleNumber = STB.StyleNumber
										INNER JOIN	Financial.dbo.Seasons AS SNB	WITH(NOLOCK) ON SNB.SeasonID = STB.SeasonID	AND SNB.SeasonID = 2229	--Season BLANK
        ...
        UPDATE AF SET
             SewingLabor_SAM		 = TL.SewLabor_SAM
            ,SewingLaborExpected_SAM = TL.SewLaborExpcted_SAM
        FROM #Report AS AF
