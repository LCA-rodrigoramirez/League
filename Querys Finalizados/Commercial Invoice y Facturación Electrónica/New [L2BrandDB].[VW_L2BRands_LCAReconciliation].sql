...
/****** Object:  View [L2BrandDB].[VW_L2BRands_LCAReconciliation]    Script Date: 09/07/2026 03:48:16 p. m. ******/
SET ANSI_NULLS ON
GO
...
ALTER   VIEW [L2BrandDB].[VW_L2BRands_LCAReconciliation] AS
...
WITH CTE_SH AS (
	SELECT
		...
		,[Invoice]				= IIF(A.Waybill LIKE 'ARD%', 'Service',COALESCE(SH.DM, MDB.DM,'TBD'))
		,[Container]			= CASE
									WHEN A.[StyleNumber] NOT IN ('-','Fabric','Trim','Supplies','SWATCH')
									THEN A.Container
									ELSE A.Waybill
	...
	INNER JOIN AppsLCA.dbo.DTE_FACTURAS_ELECTRONICAS AS FE WITH(NOLOCK) ON A.Waybill = FE.Factura AND FE.invalidado = 0 AND idReceptor IN (1,4)
	 LEFT JOIN
                (
                    SELECT DISTINCT
                        SH.[WayBill]
	...
	LEFT JOIN OPENQUERY(MARIADB,'SELECT * FROM wordpress.VW_ImpExp_Imp_Output') AS MDB ON A.Waybill = MDB.Waybill
	WHERE CAST(A.ShipDate AS DATE) >= CAST(DATEADD(DAY, 1, EOMONTH(GETDATE(), -4)) AS DATE)
	--AND 
	--	(
	--		A.[StyleNumber] IN ('-','Fabric','Trim','Supplies','SWATCH')
	--		OR 
	--		(
	--			A.[StyleNumber] NOT IN ('-','Fabric','Trim','Supplies','SWATCH')
	--			AND SH.DM IS NOT NULL
	--		)
	--	)
)
,CTE_StyleHTS AS (
