...
-- END
	...
	------------------------------------------- STYLE CERTIFICATION -------------------------------------
        ...
        SELECT
             [StyleID], [StyleNumber], [StyleColorID], [VendorName], [FabricConstruction], [AddressID]
        ...
        FROM #TB_DataCI						 AS CI
        INNER JOIN LCA.dbo.ManufactureOrders AS MO WITH(NOLOCK) ON CI.[ManufactureID] 	= MO.[ManufactureID] AND CI.[ProductDivision] <> 'Headwear'
		...
		WHERE IDVersion IS NULL OR IDVersion = 'PENDIENTE' OR CertifyID IS NULL
		...
		SELECT
			...
			,CS.CertifyID
		-- UPDATE CS SET
		-- 	CertifyID = TC.CertifyID
		-- 	,IDVersion = TC.IDVersion
		FROM #TB_Certification AS TC
		INNER JOIN [192.168.1.93].AppsLCA.dbo.CI_Import_Export_CertificationStyle AS CS ON TC.IDExport = CS.IDExport
		WHERE (TC.CertifyID <> ISNULL(CS.CertifyID,'')) OR (TC.IDVersion <> ISNULL(CS.IDVersion,''))
