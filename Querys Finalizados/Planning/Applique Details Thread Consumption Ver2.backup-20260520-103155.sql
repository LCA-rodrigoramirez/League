	...
	BEGIN TRY
		...
		/************************************************* Obteniendo info del Digitizing (Design, Sequence, Comment por Sequence) *************************************************/
			...
			SELECT
				...
				,TMC.[Location]
				,TMC.[LogoStyle]
				,SUM(CA.[CountApp]) AS [CountApp]
				,SUM(AppliquePerUnit) AS AppliquePerUnit
			FROM
			(
				SELECT
					TMC.ItemDetailID
				...
				(
					SELECT
						...
						,[DESIGN]
						,[APPLIQUE QTY]
					FROM [192.168.1.93].[AppsLCA].[dbo].[PBI_EMH_AppliquesbyCode] AS PEAC WITH(NOLOCK)
					GROUP BY
						[TYPE]
						,[DESIGN]
						,[APPLIQUE QTY]
				) AS PEAC ON TMC.[LogoStyle] = PEAC.[DESIGN] 
			...
			GROUP BY
				...
				,TMC.[Location]
				,TMC.[LogoStyle]
			HAVING SUM(CountApp) > SUM(AppliquePerUnit)
			ORDER BY TMC.ItemDetailID
			...
			(
				...
				(
					...
					LEFT JOIN
					(
						SELECT [TYPE], [DESIGN], [APPLIQUE QTY]
						FROM [192.168.1.93].[AppsLCA].[dbo].[PBI_EMH_AppliquesbyCode] WITH(NOLOCK)
						GROUP BY [TYPE], [DESIGN], [APPLIQUE QTY]
					) AS PEAC ON TMC.[LogoStyle] = PEAC.[DESIGN]
					WHERE LogoStyleName <> '-' AND (PEAC.[TYPE] <> 'PATCH' OR PEAC.[TYPE] IS NULL)
			...
			) AS DIFF ON R.ItemDetailID = DIFF.ItemDetailID AND R.[Location] = DIFF.[Location]
