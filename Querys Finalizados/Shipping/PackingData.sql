...
----- =========================================== VALIDACION UNIDADES DE WORK ORDER EN WAYBILL (ItemDetailID) ====================================================
				...
				UPDATE S SET
					...
					,[Comment]    = TV.[Comment]
					,[FlagReason] = IIF(TV.[PendingITApproval] = 1, 0, 1)
				FROM #TB_DataPacking AS S
				...
				UPDATE S SET
					...
					,[FlagReason] = IIF(TV.[PendingITApproval] = 1, 0, 1)
				FROM #TB_DataPacking AS S
				INNER JOIN #TB_WOValidation AS TV ON TV.[ItemDetailID] = S.[ItemDetailID] AND S.[ManufactureID] = TV.[ManufactureID]
				WHERE S.[Comment] IS NULL
...
----- =========================================== PROCESO PARA VALIDAR TRACKINGNO ====================================================
				...
				LEFT JOIN (
                    SELECT 
                         [R]            = ROW_NUMBER() OVER(PARTITION BY aoo.ItemDetailID ORDER BY aoo.ItemDetailID , aoo.Insert_time DESC)
                        ,[ItemDetailID] = aoo.[ItemDetailID]
                        ,[ShipTo]       = aoo.[Ship To]
                    FROM        #TB_DataPacking AS DP 
                    INNER JOIN [AppsLCA].[legacycaps].[VW_view_qryLCA_Order_Export] AS aoo WITH(NOLOCK) ON aoo.ItemDetailID = DP.ItemDetailID
				) AS B ON GP.ItemDetailID = B.ItemDetailID AND B.[R] =1

				UPDATE GP SET
					ShipTo = B.ShipTo
				FROM #TB_GROUP_ITEMDetailID_VALIDATION_TRACKING AS GP
				LEFT JOIN (
                    SELECT 
				...
				----------------------------------------------------------------------------------------------------------------------------
				-----VALIDACION SHIP TO REMOTO PARA ITEMDETAILID QUE SIGUEN SIN TRACKING (ExistTracking = 0)------------------------------------
				----------------------------------------------------------------------------------------------------------------------------
				DECLARE @ids_ItemDetail_NoTracking NVARCHAR(MAX)
				SELECT @ids_ItemDetail_NoTracking = STRING_AGG(CONVERT(NVARCHAR(MAX), ItemDetailID), ',')
				FROM #TB_GROUP_ITEMDetailID_VALIDATION_TRACKING
				WHERE ExistTracking = 0

				DROP TABLE IF EXISTS #TB_RemoteShipTo_NoTracking
				CREATE TABLE #TB_RemoteShipTo_NoTracking
				(
					 [ItemDetailID]  INT
					,[ShipTo]        VARCHAR(200)
				)

				IF @ids_ItemDetail_NoTracking IS NOT NULL
				BEGIN
					DECLARE @sqlRemoteShipTo_NoTracking NVARCHAR(MAX) = '
						SELECT ItemDetailID, [Ship To]
						FROM OPENQUERY([192.168.100.26], ''SELECT ItemDetailID, [Ship To] FROM [Production].[dbo].[view_qryLCA_Order_Export] WITH(NOLOCK) WHERE ItemDetailID IN (' + @ids_ItemDetail_NoTracking + ') GROUP BY ItemDetailID, [Ship To]'')'

					INSERT INTO #TB_RemoteShipTo_NoTracking
					EXEC (@sqlRemoteShipTo_NoTracking)
				END

				ALTER TABLE #TB_GROUP_ITEMDetailID_VALIDATION_TRACKING ADD DiffShipTo BIT NULL

				UPDATE S SET
					 DiffShipTo = 1
					,ShipTo     = R.ShipTo
				FROM #TB_GROUP_ITEMDetailID_VALIDATION_TRACKING AS S
				INNER JOIN #TB_RemoteShipTo_NoTracking AS R ON R.ItemDetailID = S.ItemDetailID
				WHERE S.ExistTracking = 0
				  AND (S.ShipTo <> R.ShipTo OR (S.ShipTo IS NULL AND R.ShipTo IS NOT NULL) OR (S.ShipTo IS NOT NULL AND R.ShipTo IS NULL))

				-- Recalcula ExistTracking tomando en cuenta el ShipTo ya corregido
				UPDATE S SET
					[ExistTracking] = CASE
										WHEN S.ShipTo = 'Hanover'  THEN 1
										WHEN S.ShipTo = 'Miami' AND B.ExistTracking > 0 THEN 1
				...
				WHERE S.DiffShipTo = 1

				-- Log histórico ANTES del update (estado previo, con su Insert_time original)
				INSERT INTO [192.168.1.93].[AppsLCA].[legacycaps].[VW_view_qryLCA_Order_Export_Logs]
				(
					 [Cust #],[Short Name],[SOP Type],[ORD #],[Item #],[Item Desc],[Brand],[Qty],[Req Ship],[Status]
					,[E or S],[Customer],[Outsource],[Return Date],[Sent Out],[Status/Date],[Doc Date],[Design],[Design Desc],[Cust PO #]
					,[Cust Class],[Cancel],[Hold Status],[Pulled],[Cust Priority],[APS#],[R/S Priority],[Unit Price],[Document Amount],[Comment 3]
					,[OriginalRequestDate],[Ship To],[SizingID],[ItemDetailID],[Shipto Name],[Address 1],[Address 2],[City],[State],[Zip]
					,[Ship Via],[Ship Acct No],[SKUID Link],[OrderNo],[Application Type],[SKU Number],[DetailStatus],[CXR Name],[OrderItemID],[License Sticker]
					,[Primary Design #],[OrderType],[OrderHOLD],[Email],[Phone],[Size],[Style],[Color],[Insert_time],[ContactName]
					,[SalesChannel],[CustDueDate],[GroupID],[EventDate],[Hot Order],[IsNovaStyle],[ShipInstructions],[OrderNote],[CustProdNote],[LogoStyleName]
					,[MachineGroup],[NewStyle],[NewColor],[NewSize],[ProductDivision],[Relabel],[Item # L2],[AcctngID],[UPC],[CustomerRetail]
					,[Item # L3],[MovedPerAllocation],[PromiseDate],[InventoryDate],[ShipCompleteYN],[DoNotOverShip],[ShipExactQty],[DoNotShipBeforeDate]
				)
				SELECT
					 aoo.[Cust #],aoo.[Short Name],aoo.[SOP Type],aoo.[ORD #],aoo.[Item #],aoo.[Item Desc],aoo.[Brand],aoo.[Qty],aoo.[Req Ship],aoo.[Status]
					,aoo.[E or S],aoo.[Customer],aoo.[Outsource],aoo.[Return Date],aoo.[Sent Out],aoo.[Status/Date],aoo.[Doc Date],aoo.[Design],aoo.[Design Desc],aoo.[Cust PO #]
					,aoo.[Cust Class],aoo.[Cancel],aoo.[Hold Status],aoo.[Pulled],aoo.[Cust Priority],aoo.[APS#],aoo.[R/S Priority],aoo.[Unit Price],aoo.[Document Amount],aoo.[Comment 3]
					,aoo.[OriginalRequestDate],aoo.[Ship To],aoo.[SizingID],aoo.[ItemDetailID],aoo.[Shipto Name],aoo.[Address 1],aoo.[Address 2],aoo.[City],aoo.[State],aoo.[Zip]
					,aoo.[Ship Via],aoo.[Ship Acct No],aoo.[SKUID Link],aoo.[OrderNo],aoo.[Application Type],aoo.[SKU Number],aoo.[DetailStatus],aoo.[CXR Name],aoo.[OrderItemID],aoo.[License Sticker]
					,aoo.[Primary Design #],aoo.[OrderType],aoo.[OrderHOLD],aoo.[Email],aoo.[Phone],aoo.[Size],aoo.[Style],aoo.[Color],aoo.[Insert_time],aoo.[ContactName]
					,aoo.[SalesChannel],aoo.[CustDueDate],aoo.[GroupID],aoo.[EventDate],aoo.[Hot Order],aoo.[IsNovaStyle],aoo.[ShipInstructions],aoo.[OrderNote],aoo.[CustProdNote],aoo.[LogoStyleName]
					,aoo.[MachineGroup],aoo.[NewStyle],aoo.[NewColor],aoo.[NewSize],aoo.[ProductDivision],aoo.[Relabel],aoo.[Item # L2],aoo.[AcctngID],aoo.[UPC],aoo.[CustomerRetail]
					,aoo.[Item # L3],aoo.[MovedPerAllocation],aoo.[PromiseDate],aoo.[InventoryDate],aoo.[ShipCompleteYN],aoo.[DoNotOverShip],aoo.[ShipExactQty],aoo.[DoNotShipBeforeDate]
				FROM [AppsLCA].[legacycaps].[VW_view_qryLCA_Order_Export] AS aoo WITH(NOLOCK)
				INNER JOIN #TB_GROUP_ITEMDetailID_VALIDATION_TRACKING AS S ON S.ItemDetailID = aoo.ItemDetailID
				WHERE S.DiffShipTo = 1

				-- Propaga el ShipTo corregido a la tabla origen
				UPDATE aoo SET
					[Ship To] = S.[ShipTo]
				FROM [AppsLCA].[legacycaps].[VW_view_qryLCA_Order_Export] AS aoo
				INNER JOIN #TB_GROUP_ITEMDetailID_VALIDATION_TRACKING AS S ON S.ItemDetailID = aoo.ItemDetailID
				WHERE S.DiffShipTo = 1

				-- Log histórico DESPUÉS del update (estado nuevo), con Insert_time = GETDATE()
				INSERT INTO [192.168.1.93].[AppsLCA].[legacycaps].[VW_view_qryLCA_Order_Export_Logs]
				(
					 [Cust #],[Short Name],[SOP Type],[ORD #],[Item #],[Item Desc],[Brand],[Qty],[Req Ship],[Status]
					,[E or S],[Customer],[Outsource],[Return Date],[Sent Out],[Status/Date],[Doc Date],[Design],[Design Desc],[Cust PO #]
				...
				SELECT
					...
					,aoo.[OriginalRequestDate],aoo.[Ship To],aoo.[SizingID],aoo.[ItemDetailID],aoo.[Shipto Name],aoo.[Address 1],aoo.[Address 2],aoo.[City],aoo.[State],aoo.[Zip]
					,aoo.[Ship Via],aoo.[Ship Acct No],aoo.[SKUID Link],aoo.[OrderNo],aoo.[Application Type],aoo.[SKU Number],aoo.[DetailStatus],aoo.[CXR Name],aoo.[OrderItemID],aoo.[License Sticker]
					,aoo.[Primary Design #],aoo.[OrderType],aoo.[OrderHOLD],aoo.[Email],aoo.[Phone],aoo.[Size],aoo.[Style],aoo.[Color],GETDATE(),aoo.[ContactName]
					,aoo.[SalesChannel],aoo.[CustDueDate],aoo.[GroupID],aoo.[EventDate],aoo.[Hot Order],aoo.[IsNovaStyle],aoo.[ShipInstructions],aoo.[OrderNote],aoo.[CustProdNote],aoo.[LogoStyleName]
					,aoo.[MachineGroup],aoo.[NewStyle],aoo.[NewColor],aoo.[NewSize],aoo.[ProductDivision],aoo.[Relabel],aoo.[Item # L2],aoo.[AcctngID],aoo.[UPC],aoo.[CustomerRetail]
					,aoo.[Item # L3],aoo.[MovedPerAllocation],aoo.[PromiseDate],aoo.[InventoryDate],aoo.[ShipCompleteYN],aoo.[DoNotOverShip],aoo.[ShipExactQty],aoo.[DoNotShipBeforeDate]
				FROM [AppsLCA].[legacycaps].[VW_view_qryLCA_Order_Export] AS aoo WITH(NOLOCK)
