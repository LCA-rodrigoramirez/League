...
--- Pre ingreso de datos en #TB_Transfer para evitar contar cajas 2 veces
	...
	(
		...
		INNER JOIN	[AppsLCA].[dbo].[TB_Transfer_Export_Duty]		AS TE	WITH(NOLOCK)	ON TE.Waybill = TB_GROUP.WayBill AND TE.BoxNumber = TB_GROUP.BoxNumber 
																																AND TE.StyleNumber = TB_GROUP.StyleNumber 
																																AND TE.StyleColor = TB_GROUP.StyleColor
																																AND TE.Size = TB_GROUP.GarmentSize
																																AND TE.[status] = 1
		INNER JOIN  [AppsLCA].[dbo].[ImportExport_AnexoFacturacion]	AS SB	WITH(NOLOCK) ON TE.ID = SB.ID
