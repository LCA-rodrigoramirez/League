<?php 
    ...
    $conn = sqlsrv_connect($serverName,$connectionOptions);
        ...
        if ($conn){
            $query = "SELECT * FROM [dboReaders].[VW_PurchaseOrdersDetails] WHERE PONumber ='".$PO."' ORDER BY PartNumber";
            ...
            $countDatos = 0; //SABER SI ESTÁ ENTRANDO AL WHILE PARA IMPRIMIR LOS DATOS DE LA CONSULTA EN LA TABLA
              ...
              while ($row = sqlsrv_fetch_array($result, SQLSRV_FETCH_ASSOC)) {
                ...
                $tabla .= "<tr>
                            ...
                            <td align = 'center'>".$row["PartNumber"]."</td>
                            <td align = 'center'>".$row["Vendor"]."</td>
                            <td align = 'center'>".$row["Ordered"]."</td>
                            <td align = 'center'>".$row["Received"]."</td>
                            <td align = 'center'>".$row["HTSCode"]."</td>
                            <td><input type='number' id='numeroCajasCompletas-".$row["PartNumber"]."' class='form-control' style='width: 70px;' placeholder='0'></td>
                            <td><input type='number' id='unidadesCajasCompletas-".$row["PartNumber"]."' class='form-control' style='width: 70px;' placeholder='0'></td>
                            <td><input type='text' id='pesoCajasCompletas-".$row["PartNumber"]."' class='form-control' style='width: 70px;' placeholder=''></td>
                            <td id='col-".$row["PartNumber"]."'><input type='number'id='numCajaBalance-".$row["PartNumber"]."' class='form-control' style='width: 70px;' placeholder='0'></td>
                            <td><button type='button' class='btn btn-primary' onclick='AñadirCeldas(this)' id='button-".$row["PartNumber"]."'>Agregar Cajas<br>de Balance</button>
                              <button type='button' class='btn btn-info' onclick='Guardar(this)' id='button-".$row["PartNumber"]."'>Guardar Item</button></td>
                        </tr>";
                }
