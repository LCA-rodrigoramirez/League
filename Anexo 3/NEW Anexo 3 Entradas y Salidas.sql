...
--,tt.CategoryName
,tt.DatabaseUnits
,SUM(CASE WHEN TT.EnumDesc in ('Receive','Receive Excess') THEN tt.Quantity ELSE 0 END) AS QuantityEntradas
,SUM(CASE WHEN tt.EnumDesc in ('Receive','Receive Excess') THEN tt.[UnitCost * Quantity] ELSE 0 END) AS costEntradas
,SUM(CASE WHEN tt.EnumDesc in ('adjust minus','adjust plus','MO Return','MO Withdraw') THEN abs(tt.Quantity) ELSE 0 END) AS QuantitySalidas
,SUM(CASE WHEN tt.EnumDesc in ('adjust minus','adjust plus','MO Return','MO Withdraw') THEN abs(tt.[UnitCost * Quantity]) ELSE 0 END) AS costSalidas
                ...
                --   RawTransactions.CutOrderID AS CutOrderID,
                 ...
                 --  RawTransactions.TransactionReasonID AS TransactionReasonID,
                   ...
                   ReceiveSlips.WayBill AS WayBill_
					...
					,EnumValues.Description as EnumDesc
...
(
    ...
    WHERE convert( varchar(10),changedate, 111) BETWEEN '2025/01/01'  AND '2025/03/31'
...
--PARA LOS CONTRACTS SOLO TOMAR EN CUENTA LAS ENTRADAS (NUEVO CRITERIO)
--LAS SALIDAS SERÁN TOMADAS DE LAS TRANSACCIONES DE LAS CAJAS
...
and (EnumValues.Description in ('Receive','Receive Excess')--ENTRADAS
 or EnumValues.Description in ('adjust minus','adjust plus','MO Return','MO Withdraw'))--SALIDAS
-- and ddv.Description = '(MUESTRARIO) LOS DEMAS TRAPOS CORDELES, CUERDAS Y CORDAJES DE MATERIA TEXTIL'
